...
public interface MessageRecordMapper extends BaseMapper<MsgRecord> {
    ...
    /**
     * 聚合查询今日和昨日记录数量。
     * 已发送记录按最近实际发送时间归属，尚未发送的记录按创建时间归属。
     */
    @Select({
            "SELECT",
            "COALESCE(SUM(CASE WHEN COALESCE(r.send_time, r.create_time) >= #{todayStart}",
            "  AND COALESCE(r.send_time, r.create_time) < #{tomorrowStart}",
            "  THEN 1 ELSE 0 END), 0) AS todayTotal,",
            "COALESCE(SUM(CASE WHEN COALESCE(r.send_time, r.create_time) >= #{todayStart}",
            "  AND COALESCE(r.send_time, r.create_time) < #{tomorrowStart}",
            ...
            "COALESCE(SUM(CASE WHEN COALESCE(r.send_time, r.create_time) >= #{todayStart}",
            "  AND COALESCE(r.send_time, r.create_time) < #{tomorrowStart}",
            "  AND r.send_status IN ('PENDING', 'ACCEPTED') THEN 1 ELSE 0 END), 0) AS todayPending,",
            "COALESCE(SUM(CASE WHEN COALESCE(r.send_time, r.create_time) >= #{yesterdayStart}",
            "  AND COALESCE(r.send_time, r.create_time) < #{todayStart}",
            "  THEN 1 ELSE 0 END), 0) AS yesterdayTotal",
            "FROM msg_record r",
            "WHERE r.deleted = 0",
            "AND COALESCE(r.send_time, r.create_time) >= #{yesterdayStart}",
            "AND COALESCE(r.send_time, r.create_time) < #{tomorrowStart}"
    })
    ...
    @Select({
            ...
            "r.user_id AS userId, r.user_name AS userName, r.user_org_id AS userOrgId,",
            "r.priority AS priority, r.call_type AS callType,",
            "r.message_content AS messageContent,",
            "r.send_status AS sendStatus, r.resend_count AS resendCount,",
            "r.max_resend_count AS maxResendCount,",
            ...
            "OR r.user_id LIKE '%' || #{query.userId} || '%'",
            "OR (COALESCE(r.user_name, '') || COALESCE(r.user_id, ''))",
            "LIKE '%' || #{query.userId} || '%'",
            ")",
            "</if>",
    ...
    @Select({
            ...
            "r.send_status AS sendStatus, r.resend_count AS resendCount,",
            "r.max_resend_count AS maxResendCount, r.error_msg AS errorMsg,",
